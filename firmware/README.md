...
```sh
cd firmware
pio run -e thinkie-rx                                   # build
pio run -e thinkie-rx -t upload --upload-port $(../tools/ports.py A)
```

Envs:
- `thinkie-rx` — Stage 1, receive-only. `-DTWR_TX_DISABLED`; `main.cpp` refuses to
  build without it. No code path touches PTT.

Protocol: `src/protocol.h` (mirrored by `../host/thinkie_link.py`).
